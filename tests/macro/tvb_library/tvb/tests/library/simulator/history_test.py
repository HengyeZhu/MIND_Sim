...
class TestsExactPropagation:
    def build_simulator(self, macro_mod_dir, n=4):
        ...
        for roi in rois:
            roi.initial_history(np.ones((n * n, 1)), outputs=["x"])
        return rois
...
def test_roi_initial_history_sets_each_roi_current_state(macro_mod_dir):
    _configure_macro_runtime(macro_mod_dir, 1.0)
    ...
    rois.roi("B").use_macro("tvb_linear", initial_state={"x": 0.0}, params={"gamma": 0.0})
    for roi in rois:
        roi.record("x")

    rois.roi("A").initial_history(np.array([[10.0], [11.0]]), outputs=["x"])
    rois.roi("B").initial_history(np.array([20.0, 21.0]), outputs="x")

    xs = _records_cube(ms.macro.Simulator(rois).run(n_steps=1))[:, :, 0]
    np.testing.assert_allclose(xs[0], [11.0, 21.0])
