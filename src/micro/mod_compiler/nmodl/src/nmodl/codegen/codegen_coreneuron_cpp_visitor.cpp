...
std::string mind_field_role_name(const std::string& name, const CodegenInfo& info) {
    const auto has_name = [&](const auto& vars) {
        return std::any_of(vars.begin(), vars.end(), [&](const auto& symbol) {
            return symbol->get_name() == name;
        });
    };
    if (has_name(info.range_parameter_vars)) {
        return "neuron::mechanism::field_role::parameter";
    }
    if (has_name(info.range_assigned_vars) || has_name(info.assigned_vars)) {
        return "neuron::mechanism::field_role::assigned";
    }
    if (has_name(info.range_state_vars) || has_name(info.state_vars)) {
        return "neuron::mechanism::field_role::state";
    }
    return "neuron::mechanism::field_role::range";
}

struct MindIonDParamMetadata {
    std::string target{};
    int conc_style{0};
    int rev_style{0};
    bool write_interior{false};
    bool write_exterior{false};
};

std::string mind_ion_storage_field_name(const Ion& ion, const std::string& variable) {
    if (ion.is_rev_potential(variable)) {
        return ion.rev_potential_name();
    }
    if (ion.is_intra_cell_conc(variable)) {
        return ion.intra_conc_name();
    }
    if (ion.is_extra_cell_conc(variable)) {
        return ion.extra_conc_name();
    }
    if (ion.is_ionic_current(variable)) {
        return ion.ionic_current_name();
    }
    if (ion.is_current_derivative(variable)) {
        return "di" + ion.name + "_dv_";
    }
    return {};
}

MindIonDParamMetadata mind_registered_dparam_metadata(const std::string& name,
                                                      const std::string& semantic,
                                                      const CodegenInfo& info) {
    MindIonDParamMetadata metadata{};
    if (!stringutils::ends_with(semantic, "_ion")) {
        return metadata;
    }
    const bool style = stringutils::starts_with(semantic, "#");
    const auto ion = style ? semantic.substr(1, semantic.size() - 5)
                           : semantic.substr(0, semantic.size() - 4);
    const auto ion_it = std::find_if(info.ions.begin(), info.ions.end(), [&](const auto& item) {
        return item.name == ion;
    });
    if (ion_it == info.ions.end()) {
        return metadata;
    }
    metadata.conc_style = ion_it->is_conc_written() ? 3 : static_cast<int>(ion_it->is_conc_read());
    metadata.rev_style = ion_it->is_rev_written() ? 3 : static_cast<int>(ion_it->is_rev_read());
    metadata.write_interior = ion_it->is_interior_conc_written();
    metadata.write_exterior = ion_it->is_exterior_conc_written();
    if (style) {
        return metadata;
    }
    if (stringutils::starts_with(name, naming::ION_VARNAME_PREFIX)) {
        metadata.target = mind_ion_storage_field_name(
            *ion_it,
            name.substr(std::string(naming::ION_VARNAME_PREFIX).size()));
    }
    return metadata;
}
...
void CodegenCoreneuronCppVisitor::print_global_param_default_values() {
    ...
    for (const auto& p: info.range_parameter_vars) {
        ...
        defaults.push_back(fmt::format("{} /* {} */",
                                       nmodl::stringutils::to_string(value),
                                       p->get_name()));
    }
...
void CodegenCoreneuronCppVisitor::print_mechanism_registration_helpers() {
    printer->add_newline();
    printer->add_multi_line(R"CODE(
        namespace {
        template <typename T>
...
void CodegenCoreneuronCppVisitor::print_mechanism_register() {
    ...
    for (int i = 0; i < codegen_float_variables_size; ++i) {
        ...
        const auto role = mind_field_role_name(float_var->get_name(), info);
    ...
    for (int i = 0; i < codegen_int_variables_size; ++i) {
        ...
        const auto metadata = mind_registered_dparam_metadata(name, info.semantics[i].name, info);
        if (metadata.target.empty() && metadata.conc_style == 0 && metadata.rev_style == 0 &&
            !metadata.write_interior && !metadata.write_exterior) {
            mech_register_args.push_back(
            ...
            mech_register_args.push_back(
                fmt::format("_nrn_mechanism_field<{}>{{\"{}\", \"{}\", \"{}\", {}, {}, {}, {}}} /* {} */",
                            ...
                            metadata.conc_style,
                            metadata.rev_style,
                            metadata.write_interior ? "true" : "false",
                            metadata.write_exterior ? "true" : "false",
                            i));
        }
    }
    ...
    for (auto& semantic: info.semantics) {
        auto args = fmt::format("mech_type, {}, {}", semantic.index, add_escape_quote(semantic.name));
    ...
    for (const auto& variable: info.global_variables) {
        if (!variable->is_array()) {
            auto field_name = add_escape_quote(variable->get_name());
            auto hoc_name = add_escape_quote(variable->get_name() + "_" + info.mod_suffix);
            auto storage_name = get_variable_name(variable->get_name(), false);
            printer->fmt_line("mind_register_global_scalar(mech_type, {}, {}, &{});",
                              field_name,
                              hoc_name,
                              storage_name);
        }
...
void CodegenCoreneuronCppVisitor::print_codegen_routines() {
    ...
    print_mechanism_registration_helpers();
