...
COMMENT
CoreNEURON-compatible VecStim mechanism adapted from the NEURON Simulator
example share/examples/nrniv/netcon/vecevent.mod.
ENDCOMMENT
...
VERBATIM
        {
            const char* path = getenv("TVB_NETPYNE_VECSTIM_EVENT_DUMP");
            if (path && path[0] != '\0') {
                FILE* f = fopen(path, "a");
                if (f) {
                    fprintf(f, "%.17g\n", t);
