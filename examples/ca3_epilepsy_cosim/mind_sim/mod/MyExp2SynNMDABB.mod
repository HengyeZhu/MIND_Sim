...
:  THREADSAFE
  ...
  RANGE washEnabled, washInT, washOutT, washInR, washOutR
...
NET_RECEIVE(w (uS)) {LOCAL ww, rr
  ww=w
  rr=r
  if (washEnabled > 0.5) {
    if (t >= washInT && t < washOutT) {
      rr = washInR
    }
    if (t >= washOutT) {
      rr = washOutR
  ...
  :printf("NMDA Spike: %g\n", t)
  if(rr>=0){ : if rr>=0, g = AMPA + NMDA*rr
    A  = A  + factor *ww
    B  = B  + factor *ww
    A2 = A2 + factor2*ww*rr
    B2 = B2 + factor2*ww*rr
  }else{
    if(rr>-1000){ : if rr>-1, g = NMDA*rr
      A2 = A2 - factor2*ww*rr
      B2 = B2 - factor2*ww*rr
    }
    : if rr<0 and rr<>-1, g = 0
  }
}
